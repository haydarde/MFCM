...
dataOrg <- read.csv("datasets/3_absenteeismUse.csv")
...
dataOrg$eth <- as.integer(as.factor(dataOrg$eth))
dataOrg$sex <- as.integer(as.factor(dataOrg$sex))
dataOrg$age <- as.integer(as.factor(dataOrg$age))
dataOrg$lrn <- as.integer(as.factor(dataOrg$lrn))

new <- 0:1
old <- 1:2
dataOrg$eth[dataOrg$eth %in% old] <- new[match(dataOrg$eth,old,nomatch = 0)]
dataOrg$sex[dataOrg$sex %in% old] <- new[match(dataOrg$sex,old,nomatch = 0)]
dataOrg$lrn[dataOrg$lrn %in% old] <- new[match(dataOrg$lrn,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data3.RData")
load("distMats/distanceObs_data3.RData")
