...
dataOrg <- read.csv("datasets/8_flower.csv")
...
dataOrg$V1 <- as.integer(dataOrg$V1)
dataOrg$V2 <- as.integer(dataOrg$V2)
dataOrg$V3 <- as.integer(dataOrg$V3)
dataOrg$V4 <- as.integer(dataOrg$V4)
dataOrg$V5 <- as.integer(dataOrg$V5)
dataOrg$V6 <- as.integer(dataOrg$V6)

new <- 0:1
old <- 1:2
dataOrg$V1[dataOrg$V1 %in% old] <- new[match(dataOrg$V1,old,nomatch = 0)]
dataOrg$V2[dataOrg$V2 %in% old] <- new[match(dataOrg$V2,old,nomatch = 0)]
dataOrg$V3[dataOrg$V3 %in% old] <- new[match(dataOrg$V3,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data8.RData")
load("distMats/distanceObs_data8.RData")
