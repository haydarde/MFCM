...
dataOrg <- read.csv("datasets/11_FairUse.csv")
...
dataOrg$child <- as.integer(as.factor(dataOrg$child))

new <- 0:1
old <- 1:2
dataOrg$sex[dataOrg$sex %in% old] <- new[match(dataOrg$sex,old,nomatch = 0)]
dataOrg$child[dataOrg$child %in% old] <- new[match(dataOrg$child,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data11.RData")
load("distMats/distanceObs_data11.RData")
