...
new <- 0:1
old <- 1:2
dataOrg$X2[dataOrg$X2 %in% old] <- new[match(dataOrg$X2,old,nomatch = 0)]
dataOrg$X3[dataOrg$X3 %in% old] <- new[match(dataOrg$X3,old,nomatch = 0)]
dataOrg$X4[dataOrg$X4 %in% old] <- new[match(dataOrg$X4,old,nomatch = 0)]
dataOrg$X5[dataOrg$X5 %in% old] <- new[match(dataOrg$X5,old,nomatch = 0)]
dataOrg$X6[dataOrg$X6 %in% old] <- new[match(dataOrg$X6,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data5.RData")
load("distMats/distanceObs_data5.RData")
