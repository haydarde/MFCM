...
dataOrg <- read.csv("datasets/10_HousePricesUse.csv")
...
dataOrg$driveway <- as.integer(as.factor(dataOrg$driveway))
dataOrg$recreation <- as.integer(as.factor(dataOrg$recreation))
dataOrg$fullbase <- as.integer(as.factor(dataOrg$fullbase))
dataOrg$gasheat <- as.integer(as.factor(dataOrg$gasheat))
dataOrg$aircon <- as.integer(as.factor(dataOrg$aircon))

dataOrg$stories <- as.numeric(dataOrg$stories)

new <- 0:1
old <- 1:2
dataOrg$driveway[dataOrg$driveway %in% old] <- new[match(dataOrg$driveway,old,nomatch = 0)]
dataOrg$recreation[dataOrg$recreation %in% old] <- new[match(dataOrg$recreation,old,nomatch = 0)]
dataOrg$fullbase[dataOrg$fullbase %in% old] <- new[match(dataOrg$fullbase,old,nomatch = 0)]
dataOrg$gasheat[dataOrg$gasheat %in% old] <- new[match(dataOrg$gasheat,old,nomatch = 0)]
dataOrg$aircon[dataOrg$aircon %in% old] <- new[match(dataOrg$aircon,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data10.RData")
load("distMats/distanceObs_data10.RData")
