...
dataOrg <- read.csv("datasets/9_CPS1985Use.csv")
...
dataOrg$ethnicity <- as.integer(as.factor(dataOrg$ethnicity))
dataOrg$region <- as.integer(as.factor(dataOrg$region))
dataOrg$gender <- as.integer(as.factor(dataOrg$gender))
dataOrg$occupation <- as.integer(as.factor(dataOrg$occupation))
dataOrg$sector <- as.integer(as.factor(dataOrg$sector))
dataOrg$union <- as.integer(as.factor(dataOrg$union))
dataOrg$married <- as.integer(as.factor(dataOrg$married))

new <- 0:1
old <- 1:2
dataOrg$region[dataOrg$region %in% old] <- new[match(dataOrg$region,old,nomatch = 0)]
dataOrg$gender[dataOrg$gender %in% old] <- new[match(dataOrg$gender,old,nomatch = 0)]
dataOrg$union[dataOrg$union %in% old] <- new[match(dataOrg$union,old,nomatch = 0)]
dataOrg$married[dataOrg$married %in% old] <- new[match(dataOrg$married,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data9.RData")
load("distMats/distanceObs_data9.RData")
