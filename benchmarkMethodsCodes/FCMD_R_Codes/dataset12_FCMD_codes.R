...
dataOrg <- read.csv("datasets/12_CES11Use.csv")
...
dataOrg$abortion <- as.integer(as.factor(dataOrg$abortion))
dataOrg$location <- as.integer(as.factor(dataOrg$location))
dataOrg$province <- as.integer(as.factor(dataOrg$province))
dataOrg$importance <- as.integer(as.factor(dataOrg$importance))
dataOrg$education <- as.integer(as.factor(dataOrg$education))

new <- 0:1
old <- 1:2
dataOrg$gender[dataOrg$gender %in% old] <- new[match(dataOrg$gender,old,nomatch = 0)]
dataOrg$abortion[dataOrg$abortion %in% old] <- new[match(dataOrg$abortion,old,nomatch = 0)]
dataOrg$location[dataOrg$location %in% old] <- new[match(dataOrg$location,old,nomatch = 0)]
...
for (s in 1:S){
  q[s,] <- sample(1:n,C)
  init.medoids[[s]] <- as.matrix(x[[s]][q[s,],]) # Randomly selected initial medoids for each variable type
}
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data12.RData")
load("distMats/distanceObs_data12.RData")
