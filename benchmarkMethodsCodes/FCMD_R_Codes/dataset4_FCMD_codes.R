...
dataOrg <- read.csv("datasets/4_tipsUse.csv")
...
dataOrg$smoker <- as.integer(as.factor(dataOrg$smoker))
dataOrg$day <- as.integer(as.factor(dataOrg$day))
dataOrg$time <- as.integer(as.factor(dataOrg$time))

new <- 0:1
old <- 1:2
dataOrg$sex[dataOrg$sex %in% old] <- new[match(dataOrg$sex,old,nomatch = 0)]
dataOrg$smoker[dataOrg$smoker %in% old] <- new[match(dataOrg$smoker,old,nomatch = 0)]
dataOrg$time[dataOrg$time %in% old] <- new[match(dataOrg$time,old,nomatch = 0)]
...
load("distMats/distanceObs_data4.RData")
