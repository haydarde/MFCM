...
dataOrg <- read.csv("datasets/6_us_rent_incomeUse.csv")
...
dataOrg$NAME <- as.integer(as.factor(dataOrg$NAME))
dataOrg$variable <- as.integer(as.factor(dataOrg$variable))

new <- 0:1
old <- 1:2
dataOrg$variable[dataOrg$variable %in% old] <- new[match(dataOrg$variable,old,nomatch = 0)]
...
# }
# save(distanceObs,file = "/distMats/distanceObs_data6.RData")
load("distMats/distanceObs_data6.RData")
