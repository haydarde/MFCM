...
MFCM <- function (X, varStatus = NULL, weightStatus = NULL, contDist = "Euclidean",
                         k = c(2:10), m = 2, RS = 1, stand = 1, startU = NULL, 
  ...
  out = list(U = U.opt, H = H.opt, F = NULL, clus = clus, medoid = NULL, 
             ...
             Xca = X, X = Xraw, D = NULL, index = main$index, call = match.call())
  class(out) = c("fclust")
  return(out)
...
runAllMethods <- function(data, c, m, varStatus, weightStatus, allRanges, contVars, nomVars, binVars, ordVars, 
                          distanceMatrix, mixDistanceMatrix, scale = FALSE, scaleCont = FALSE, seed = 1234, DoAll = TRUE , RS = 1){
  tictoc::tic(quiet = TRUE)
  if (scale == TRUE){
    MFCMdata <- scale(data)
    if (scaleCont == TRUE){
  ...
  clustMFCM <- MFCM(MFCMdata, varStatus = varStatus, weightStatus = weightStatus,
                    contDist = "Euclidean", k = c , m = m, RS = RS, index = "SIL.F", 
                    stand = 1, seed = seed, range = allRanges)

  cat("SIL.F for MFCM with", c, " clusters:", clustMFCM$index, "\n")
  a <- tictoc::toc(quiet = TRUE)
  cat("MFCM done in", a$toc - a$tic, "seconds ...", "\n")
  if (DoAll == FALSE){
    clsStats <- list()
    clsStats[[1]] <- cqcluster.stats(d = mixDistanceMatrix, clustering = as.integer(clustMFCM$clus[,1]))
